...
class GeneratedFile {
  const GeneratedFile(this.file, this.content);
...
///
/// Outputs include .dart, and .pbjson.dart.
class FileGenerator extends ProtobufContainer {
  ...
  List<GeneratedFile> generateFiles(OutputConfiguration config) {
    if (!_linked) throw StateError('not linked');

    GeneratedFile makeFile(String extension, String content) {
      var protoUrl = Uri.file(descriptor.name);
      var dartUrl = config.outputPathFor(protoUrl, extension);
      return GeneratedFile(dartUrl.path, content);
    }
    ...
    void imports() {
      if (enumCount > 0) {
        ...
        importWriter.addImport(_protobufImportUrl,
            prefix: protobufImportPrefix);
      }
    ...
    generateMainFile(out, importWriter, imports, config);

    final files = [
      makeFile('.dart', out.toString()),
  ...
  /// Writes the contents of the .dart file for this .proto file.
  void generateMainFile(
      IndentingWriter out, ImportWriter importWriter, void Function() imports,
      [OutputConfiguration config = const DefaultOutputConfiguration()]) {
    if (!_linked) throw StateError('not linked');

    prepareMainHeader(out, importWriter, config);
  ...
  void prepareMainHeader(IndentingWriter out, ImportWriter importWriter,
      [OutputConfiguration config = const DefaultOutputConfiguration()]) {
    ...
    for (var publicDependency in descriptor.publicDependency) {
      _addExport(importWriter, config,
          Uri.file(descriptor.dependency[publicDependency]), '.dart');
    }
  }
  ...
  /// Returns the number of enum types generated in the .dart file.
  int get enumCount {
  ...
  void generateGrpcFile(ImportWriter importWriter,
      [OutputConfiguration config = const DefaultOutputConfiguration()]) {
    if (!_linked) throw StateError('not linked');
  ...
  String generateJsonFile(
    ...
    var importWriter = ImportWriter();
    if (messageGenerators.isNotEmpty || serviceGenerators.isNotEmpty) {
      importWriter.addImport(_convertImportUrl, prefix: _convertImportPrefix);
      importWriter.addImport(_coreImportUrl, prefix: coreImportPrefix);
      importWriter.addImport(_typedDataImportUrl,
          prefix: _typedDataImportPrefix);
    }
...
''');
  ...
  void _addImport(ImportWriter importWriter, OutputConfiguration config,
    ...
    if ((ext == '.dart') || protoFileUri != target.protoFileUri) {
